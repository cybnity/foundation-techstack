...
| INDEXING INFRASTRUCTURE | Contents Indexing | Apache Solr | `WIN` | | | Lucene indexing and query of time series contents (e.g logs, backups), documents (e.g pdf) |
| PERSISTENCE INFRASTRUCTURE | Activity / Event Logs Store | [InfluxDB](https://www.influxdata.com/) | `WIN` | | | Time series data platform as NoSQL database for backup of event streams (write optimized) efficient for event logging |
...
| PERSISTENCE INFRASTRUCTURE | In-Memory Data Grid | Infinispan | X | | In-memory distributed data store storing, managing, and processing data; [Doc](https://infinispan.org/) |
...
| MONITORING INFRASTRUCTURE | APIs Monitoring | OpenTelemetry | X | | | |
| MONITORING INFRASTRUCTURE | APIs Monitoring | Jaeger | X | | | |
| OPERATING INFRASTRUCTURE | Linux Operating System | Ubuntu 64bits kernel | `WIN` | | | LTS minimal optimized version, security patch and vulnerability monitoring by Canonical |
...
| [Open Telemetry](https://grafana.com/oss/opentelemetry) |Tools, APIs, DSKs to instrument, generate, collect and export telemetry data (e.g metrics, logs, traces)| | |
| Promotheus |Pull-based system as monitoring solution allowing applications to publish the required metrics fetched by Promotheus periodically; key/value datastore utilizing LevelDB for the indices and ecah metric is stored in its own file<br>**ADV03:** compatible with Grafana for data visualization; PromQL for query; alert manager with notification tasks (deduplication, silencing, grouping features)|**Advantage:** take less disk space than InfluxDB; focused on metrics recording<br>**Disadvantage:** independant server without multi-nodes support to support load growth|`OK for metrics recording only`|
...
| Consul | | | | | | | |
| Linux Ubuntu (minimal for Docker)| | | | | | | |
| Docker | | | | | | | |
| Kubernetes | | | | | | | |
